(function ($, window, document, undefined) {
    ...
    var methods = {
        ...
        close: function () {
            ...
            var tempPath = path.substring(0, path.substr(1).indexOf('/') + 1)
            if (tempPath == '/center') {
                var sessionUrl = "/center/dashboard/sessionHint";
            } else {
                var sessionUrl = tempPath + "/center/dashboard/sessionHint";
            }
            //存放session  此次登录当前页面 不在提示。
            $.get(sessionUrl, {path: path}, function (res) {
        ...
        confirm: function () {
            ...
            var tempPath = path.substring(0, path.substr(1).indexOf('/') + 1)
            if (tempPath == '/center') {
                var url = "/center/dashboard/notHintNext";
                var sessionUrl = "/center/dashboard/sessionHint";
            } else {
                var url = tempPath + "/center/dashboard/notHintNext";
                var sessionUrl = tempPath + "/center/dashboard/sessionHint";
            }
            ...
            MConfirm('确定关闭之后，当前页面不再提示？', '', '', function (result) {
                if (result) {
                    $.get(url, {path: path}, function (res) {
    ...
    function buildWalkthrough() {
        ...
        if (step.popup.type !== "modal" && step.popup.type !== "nohighlight") {
            ...
            $("<div>").addClass("overlay-hole").height(Math.min(maxHeight, targetElement.outerHeight()) + 18).width(targetElement.outerWidth() + 18).css({
                padding: "20px",
                position: "absolute",
                top: targetElement.offset().top - 12,
    ...
    function showTooltip() {
        ...
        switch (step.popup.position) {
            ...
            case "bottom":
                ...
                break;
            case "left":
                top = overlayHoleTop - arrow / 2 + parseInt(step.popup.offsetVertical, 10)-20;
    ...
    $(document).on("click", "#jpwClose", methods.confirm);
    $(document).on("click",  "#jpwFinish", methods.close);
