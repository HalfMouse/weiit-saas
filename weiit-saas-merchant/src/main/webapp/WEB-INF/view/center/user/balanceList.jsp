...
<div class="content-wrapper">
			...
			<div class="page-title" style="height:70px;padding-top:20px;">
				<h4>
					<i class="icon-arrow-left52 position-left"></i> <span
						class="text-semibold"> 会员余额记录 </span>
				</h4>
			</div>
				...
				<ul class="breadcrumb-elements"> 
					<li class="dropdown">
						...
						<ul class="dropdown-menu dropdown-menu-right">
							...
							<li class="checkbox"><label><input type="checkbox" class="styled" checked="checked" value="balance">变动余额</label></li>
							<li class="checkbox"><label><input type="checkbox" class="styled" checked="checked" value="last_balance">变动后余额</label></li>
							<li class="checkbox"><label><input type="checkbox" class="styled" checked="checked" value="type">类型</label></li>
	...
	<div class="content">
		...
		<div class="panel panel-flat">
				...
				<form class="form-horizontal" name="searchform" id="searchform" action="${pageContext.request.contextPath}/center/user/balanceList" method="post">
					...
					<table id="data-table" class="table">
						<thead>
							<tr >
								...
								<th class="balance">变动余额</th>
								<th class="last_balance">变动后余额</th>
								<th class="type">类型</th>
						...
						<tbody>
			         	...
			         	<c:forEach items="${pageInfo.list}" var="user">
							<tr>
							   ...
							   <td>
							   <c:if test="${user.state==0 }"><span class='text-success text-bold'>+${user.balance }</span></c:if>
							   <c:if test="${user.state==-1 }"><span class='text-danger text-bold'>${user.balance }</span></c:if>
							   </td>
							   <td><span class='text-success text-bold'>${user.last_balance }</span></td>
							   <td> 
	                       			<c:if test="${user.type==1}">充值</c:if>
	                            	<c:if test="${user.type==2}">后台充值</c:if>
	                            	<c:if test="${user.type==3}">订单退款</c:if>
	                           </td>	
