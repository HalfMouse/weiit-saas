	...
	<body>
		...
		<div class="content">
			...
			<ul class="head_ul">
				...
				<li class="head_li Select">被关注时回复</li>
			</ul>
		...
		<script>
			$(function() {
				...
				$('.Reply_switch').click(function() {
					if($('.switch_ul').css('margin-left') == '-21px') {
						window.location = "${pageContext.request.contextPath}/center/weixin/weixinReply/updateState?action_type=1&reply_type_id=${reply.replyTypeMap.id}&state=1";
					} else {
						window.location = "${pageContext.request.contextPath}/center/weixin/weixinReply/updateState?action_type=1&reply_type_id=${reply.replyTypeMap.id}&state=-1";
					}
				});
			...
			function buttonDelete(){
				MConfirm('删除后，关注该公众号的用户将不再接收该回复，确定删除？','','',function(result) {
					if(result){
						window.location="${pageContext.request.contextPath}/center/weixin/weixinReply/remove?action_type=1&reply_id=${reply.replyMap.reply_id}";
					}
				});
			}
		</script>
