...
<%@include file="../common/header.jsp" %>
					...
					<div class="sidebar-category">
						...
						<div class="category-content no-padding">
							<ul class="navigation navigation-alt navigation-accordion">
								...
								<li class="active"><a href="${pageContext.request.contextPath}/center/productGroup/list"><i class="icon-paragraph-justify2"></i> 商品分组</a></li>
											...
											<tr>
												...
												<td>
													<c:if test="${info.state == 0 }">
						                           		<a href="javascript:;" title="点击隐藏" class="btn btn-success m-r-5 m-b-5"
															...
															onclick="mUpdate(${info.validate_id},'${info.validate_id_token}','${info.group_name}',-1)">显示</a>
						                           	</c:if>
						                           	<c:if test="${info.state==-1}">
														<a href="javascript:;" title="点击显示" class="btn btn-warning m-r-5 m-b-5"
															style="height: 22px; padding-top: 0px;"
															onclick="mUpdate(${info.validate_id},'${info.validate_id_token}','${info.group_name}',0)">隐藏</a>
													</c:if>
												</td>
												<td style="padding:0px;margin:0px;text-align:center;">
												   	<a href="javascript:;" title="编辑" onclick="WIedit('${info.validate_id}','${info.validate_id_token}')" >
														编辑</a>&nbsp;|&nbsp;
						                       		<a href="javascript:;" title="删除" onclick="WIremove('${info.validate_id}','${info.validate_id_token}','${info.group_name}')" >
														删除</a>
												</td>
											</tr>	
										</c:forEach> 
			...
			function WIADD(){
				var url="${pageContext.request.contextPath}/center/productGroup/add?ts="+new Date().getTime();	
				commonDialogModal("商品分组新增",url,500,300,'','');
			}
			
			function WIedit(id,token){
				var url="${pageContext.request.contextPath}/center/productGroup/edit?validate_id="+id+"&validate_id_token="+token+"&ts="+new Date().getTime();	
			...
			function mUpdate(id,token,name,state){
				...
				MConfirm(title,'','',function(result) {
					if(result){
						window.location="${pageContext.request.contextPath}/center/productGroup/save?state="+state+"&validate_id="+id+"&validate_id_token="+token;
					}
				});
			}
			...
			function WIremove(id,token,name){
				MConfirm('确定要删除商品分组<label style="color: red;">'+name+'</label> 吗？','','',function(result) {
					if(result){
						window.location="${pageContext.request.contextPath}/center/productGroup/remove?validate_id="+id+"&validate_id_token="+token;
					}
				});
			}
			</script>
