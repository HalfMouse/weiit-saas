...
<%@include file="../common/header.jsp" %>
					...
					<div class="sidebar-category">
						...
						<div class="category-content no-padding">
							<ul class="navigation navigation-alt navigation-accordion">
								...
								<li class="active"><a href="${pageContext.request.contextPath}/center/productTag/list"><i class="icon-price-tags"></i> 商品标签</a></li>
					...
					<div class="page-header-content">
						<div class="page-title" style="height:70px;padding-top:20px;">
							<h4>
								...
								<span class="text-semibold">商品标签列表 </span>
							</h4>
						</div>
						<div class="heading-elements"> 
							<ul class="breadcrumb-elements"> 
								<li><button type="button"class="btn btn-info" style="border-radius:0;" onclick="WIADD()" ><i class="icon-add position-left"></i>新建标签</button></li>
							</ul>
						</div>
					</div>
				</div>
			...
			function WIADD(){
				...
				commonDialogModal("商品标签新增",url,500,300,'','');
			}
			...
			function WIedit(id,token){
				...
				commonDialogModal("商品标签编辑",url,500,300,'','');
			}

			function mUpdate(id,token,name,state){
				var title="";
				if(state=="-1"){
					title='确定要隐藏商品标签<label style="color: red;">'+name+'</label> 吗？';
				}
				if(state=="0"){
					title='确定要显示商品标签<label style="color: red;">'+name+'</label> 吗？';
				}
			...
			function WIremove(id,token,name){
				MConfirm('确定要删除商品标签<label style="color: red;">'+name+'</label> 吗？','','',function(result) {
					if(result){
