...
<%@include file="../common/header.jsp" %>
				...
				<div class="sidebar-content">
					...
					<div class="sidebar-category">
						<div class="category-content">
							<a href="${pageContext.request.contextPath}/center/product/releaseOne?saleOld=3" class="btn bg-orange btn-rounded btn-block btn-xs">发布商品</a>
						</div>
						<div class="category-content no-padding">
							<ul class="navigation navigation-alt navigation-accordion">
								...
								<li  class="active"><a href="${pageContext.request.contextPath}/center/product/forSaleList"><i class="icon-make-group"></i>仓库中的商品</a></li>
			...
			<div class="content-wrapper">
						...
						<div class="page-title" style="height:70px;padding-top:20px;">
							<h4>
								<i class="icon-arrow-left52 position-left"></i> <span
									class="text-semibold"> 仓库中的商品 </span>
							</h4>
						</div>
						<div class="heading-elements" >
							<ul class="breadcrumb-elements">
								...
								<li class="dropdown">
									...
									<ul class="dropdown-menu dropdown-menu-right">
										<li class="checkbox"><label><input type="checkbox" class="styled" checked="checked" value="forSalecolumn1">商品</label></li>
										<li class="checkbox"><label><input type="checkbox" class="styled" checked="checked" value="forSalecolumn2">商品分类</label></li> 
										<li class="checkbox"><label><input type="checkbox" class="styled" checked="checked" value="forSalecolumn4">市场价</label></li>
										<li class="checkbox"><label><input type="checkbox" class="styled" checked="checked" value="forSalecolumn5">销售价</label></li> 
										<li class="checkbox"><label><input type="checkbox" class="styled" checked="checked" value="forSalecolumn7">库存</label></li>		
										<li class="checkbox"><label><input type="checkbox" class="styled" checked="checked" value="forSalecolumn8">销量</label></li>
										<li class="checkbox"><label><input type="checkbox" class="styled" checked="checked" value="forSalecolumn9">操作</label></li>
									</ul>
								</li>
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						...
						<form class="form-horizontal"  name="searchform" id="searchform" action="${pageContext.request.contextPath}/center/product/forSaleList" method="post" >
							...
							<table  id="data-table" class="table">
								<thead>
									<tr>
										...
										<th class="forSalecolumn1">商品</th>
										<th class="forSalecolumn2">商品分类</th> 
										<th class="forSalecolumn4">市场价</th>
										<th class="forSalecolumn5">销售价</th> 
			                           	<th class="forSalecolumn7">库存</th>				                                   
			                          	<th class="forSalecolumn8">销量</th> 
										<th class="forSalecolumn9" width="200">操作</th>
									</tr>
								</thead>
								<tbody>
								<c:forEach items="${pageInfo.list}" var="info">
				                    <tr>
				                        ...
				                        <td style="width:300px;padding-left:0px;padding-right:0px;">
											<table><tr>
											<td><img src="${imageDomain }${info.product_img}?${i100}" width="60px" height="60px"></td>
											<td><a href="javascript:;" title="点击查看商品详情" onclick="detail('${info.product_id}','${info.validate_id_token }')" >
													${info.product_name }</a>
				                        ...
				                        <td style="padding:0px;margin:0px;text-align:center;">  
											 <a class="btn bg-orange-400 m-r-5 m-b-5" style="height: 22px;padding-top: 0px;" href="javascript:;" title="编辑" onclick="WIedit('${info.product_id}','${info.validate_id_token }')" >
												编辑</a>
											 <a class="btn bg-info-300 m-r-5 m-b-5" style="height: 22px;padding-top: 0px;" href="javascript:;" title="上架" onclick="WIStatus('${info.validate_id}','${info.product_name}','0','${info.validate_id_token }')" >
												上架</a>
											 <a class="btn bg-danger-300 m-r-5 m-b-5" style="height: 22px;padding-top: 0px;" href="javascript:;" title="删除" onclick="WIremove('${info.product_id}','${info.product_name}','${info.validate_id_token }')" >
												删除</a>
						...
						<div id="pager" class="panel-body">
							...
							<button type="button" class="btn btn-default" onclick="BatchOnSale(0)">
			                    	<i class="icon-square-up"></i> 批量上架</button>
			...
			function WIedit(id,validate_id_token){
				window.location="${pageContext.request.contextPath}/center/product/releaseTwo?saleOld=3&validate_id="+id+"&validate_id_token="+validate_id_token+"&ts="+new Date().getTime();	
			}
			 
			function WIStatus(id,name,status,validate_id_token){ 
				var url="${pageContext.request.contextPath}/center/product/status?saleOld=3&is_on_sale="+status+"&validate_id="+id+"&validate_id_token="+validate_id_token;
				Mremove("get",url,'确定要上架商品 <label style="color: red;">'+name+'</label> 吗？');
			}
			
			function BatchOnSale(onSale){
				var url="${pageContext.request.contextPath}/center/product/status?mfalg=1&saleOld=3&is_on_sale="+onSale;
				MremoveBath("input[name='ids']",url,'确定要批量上架选择的商品吗？');
			}
			
			function WIremove(id,name,validate_id_token){
				MConfirm('确认要删除商品  <label style="color: red;">'+name+'</label> 吗？','','',function(result) {
					if(result){
						window.location="${pageContext.request.contextPath}/center/product/status?saleOld=3&is_deleted=-1&validate_id="+id+"&validate_id_token="+validate_id_token;
					}
				});
			} 

			function BatchWIremove(){
				var url="${pageContext.request.contextPath}/center/product/status?mfalg=1&saleOld=3&is_deleted=-1";
				MremoveBath("input[name='ids']",url,'确认要批量删除选择的商品吗？');
			}
			...
			function WIeditTag(id,validate_id_token){
				commonDialogModal("商品打标签","${pageContext.request.contextPath}/center/productTag/editProductTag?saleOld=3&validate_id="+id+"&validate_id_token="+validate_id_token,800,500);	
			}
			function BatchWIeditTag(){
				...
				commonDialogModal("商品批量打标签","${pageContext.request.contextPath}/center/productTag/editProductTag?saleOld=3&ids="+ids ,800,500);	
			}
			</script>
