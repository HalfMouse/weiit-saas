...
public class InitServiceImpl extends AbstractService implements InitService {
    ...
    public void insertInit(FormMap formMap){
    	//创建店铺 
    	initMapper.insert(formMap); 
    	int shop_id=formMap.getInt("shop_id");
    	
    	//创建店铺认证信息
    	E shopCert=initMapper.selectShopCert(formMap);
    	if(shopCert==null){//店铺认证如果不存在，先初始化
    		FormMap paramMap=new FormMap();
        	paramMap.put("auth_type", "2");
        	paramMap.put("shop_id", shop_id);
    		initMapper.insertCert(paramMap);
    	}
    	
    	//获取要初始化的行业的数据编号
    	formMap.put("demo_id", formMap.get("business_id")); 
    	//插入默认余额支付
