<template>
    <div class="shopCart">
        ...
        <div v-show="!isLoading">
                    ...
                    <li>
                        <div class="goods_information clearfix">
                            ...
                            <div class="goods_main">
                                ...
                                <span>￥{{integralInfo.integral_price}} + {{integralInfo.integral}}积分</span>
            ...
            <footer>
                <div class="price">
                    <h4>实付款：<span>￥{{(integralInfo.integral_price * goodsNumber) + express_money}}</span></h4>
                    <p>邮费：￥{{express_money}}</p>
                </div>
                <!-- <a href="javascript:;" @click="share = true">找人代付</a> -->
                <a href="javascript:;" @click="submit('partake')" :style="{background:color1,color:'#fff'}">立即支付</a>
            </footer>
...
<script>
    ...
    export default {
        ...
        methods:{
            // 获取传递过来的信息
            getData(){
                let IntegralGoods = JSON.parse(localStorage.getItem('IntegralGoods'));
                this.goodsNumber = IntegralGoods.count;
                this.max_limit = IntegralGoods.integralInfo.max_limit;
                this.stock = IntegralGoods.information.stock;
                this.color1 = localStorage.getItem('bg1');
                this.validate_id = IntegralGoods.validate_id;
                this.validate_id_token = IntegralGoods.validate_id_token;
                this.product_id = IntegralGoods.information.product_id;
                this.productInfo = IntegralGoods.information;
                this.integralInfo = IntegralGoods.integralInfo;
            },
            // 选择收件人信息
            consignee(){
                let length;
                ...
                if(address_id){
                    ...
                    if(payment_type == '0' && this.wxPayAble){
                        if(this.isFlag){
                            this.isFlag = false;
                            this.isPay = true;
                            axios.post('/api/integral/integralOrder',qs.stringify({
                                validate_id: this.validate_id,
                                validate_id_token: this.validate_id_token,
