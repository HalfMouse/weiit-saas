...
Page({
        ...
        success: function(res) {
          ...
          if (res.data != "" && res.data.cart_info != '') {
            for (var i = 0; i < res.data.cart_info.length; i++) {
              res.data.cart_info[i].checked = false
            }
          };
          that.setData({
  ...
  Choice: function(e) {
    ...
    if (this.data.car_content.cart_info[choice_index].checked == false) {
      this.setData({
        [str]: true
      })
    } else {
    ...
    if (that.data.selectCoupon == "") {
      ...
      if (that.data.selectCoupon.type == 2) {
        ...
        if (that.data.selectCoupon.product_range_type == "2") {
          ...
          var specialId = [];
          console.log(that.data.commodity_arr)
  ...
  add_num: function(e) {
    ...
    if (that.data.car_content.cart_info[num_index].max_limit != 0 && that.data.car_content.cart_info[num_index].count >= that.data.car_content.cart_info[num_index].max_limit) {
      ...
      if (that.data.car_content.cart_info[num_index].count < that.data.car_content.cart_info[num_index].stock) {
        ...
        wx.showToast({
          title: '最多选择' + that.data.car_content.cart_info[num_index].stock + '件商品',
          icon: 'none',
          duration: 1000
        })
      };
  ...
  getValue: function(e) {
    ...
    // if (this.data.balancePayAble && this.data.wxPayAble){
    //   index = e.currentTarget.dataset.index;
    //   var is_radio = 'payment_method[' + index + '].is_radio';
    //   for (var i = 0; i < this.data.payment_method.length; i++) {
    //     this.data.payment_method[i].is_radio = false;
    //   }
    // } else if (!this.data.balancePayAble && this.data.wxPayAble){
    //   index = 0
    // } else if (this.data.balancePayAble && !this.data.wxPayAble){
    //   index = 1
    // }
    payment_method.forEach((item, icount) => {
      item.is_radio = false;
      if (index == icount){
        item.is_radio = true;
      }
  ...
  sub_order: function() {
    ...
    if (that.data.sub) {
      ...
      var coupon_id = that.data.selectCoupon.coupon_id;
      if (!coupon_id) {
        coupon_id = ''
      }
  ...
  getpaymentTypeInfo:function(){
    ...
    requ.request({
      ...
      success:function(res){
        ...
        if(res.code == '0'){
          that.setData({
            balancePayAble: res.data.balancePayAble,
            wxPayAble: res.data.wxPayAble
          })
          
          if (res.data.balancePayAble && res.data.wxPayAble){
            that.setData({
              payment_method: [{
                is_radio: true,
                imgs: '../../images/wechat.png',
          ...
          } else if (!res.data.balancePayAble && res.data.wxPayAble){
            that.setData({
              payment_method: [{
                is_radio: true,
                imgs: '../../images/wechat.png',
